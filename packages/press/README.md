...
WordPress is installed in public/wp, so it can be called like so: https://domain/wp

For Herd and Valet use one of the LocalValetDrivers from the wsconfig folder.

For NGINX add this after `location /`
...
// might be tooo much, Laravel not only dotenv
