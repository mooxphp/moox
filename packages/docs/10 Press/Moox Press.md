...
Current solution in public/index.php:

```php
if (preg_match('#^/wp(-admin|-[^/]*\.php|/.*|$)#', $_SERVER['REQUEST_URI'])) {
    require __DIR__.'/wp/index.php';
    exit;
}
```

possible alternative for Apache (.htaccess):
...
location ~ ^/wp/(wp-admin|wp-login\.php|wp-content|wp-includes) {
...
| **Field**             | **Type**            | **Description**                               | **Mapping**                   | **Exists** |
| --------------------- | ------------------- | --------------------------------------------- | ----------------------------- | ---------- |
| ID                    | bigint(20) unsigned | Primärschlüssel                               | id                            | Yes        |
| post_author           | bigint(20) unsigned | User-ID des Autors                            | author_id                     | Yes        |
| post_date             | datetime            | Veröffentlichungsdatum (lokal)                | published_at (timezone)       | Calc       |
| post_date_gmt         | datetime            | Veröffentlichungsdatum GMT                    | published_at                  | Yes        |
| post_content          | longtext            | Volltext / Body-Inhalt                        | content                       | Yes        |
| post_title            | text                | Titel                                         | title                         | Yes        |
| post_excerpt          | text                | Auszug / Teaser                               | description                   | Yes        |
| post_status           | varchar(20)         | z. B. publish, draft, private                 | translation_status (status)   | Yes        |
| comment_status        | varchar(20)         | open / closed                                 | comment_status                | Create     |
| ping_status           | varchar(20)         | Pingback/Trackback erlaubt                    | ❌                             | Skip       |
| post_password         | varchar(255)        | Passwortschutz                                | password                      | Create     |
| post_name             | varchar(200)        | Slug / Permalink                              | slug                          | Yes        |
| to_ping               | text                | Liste URLs zum Pingen                         | ❌                             | Skip       |
| pinged                | text                | Bereits gepingte URLs                         | ❌                             | Skip       |
| post_modified         | datetime            | Zuletzt geändert                              | updated_at (timezone)         | Calc       |
| post_modified_gmt     | datetime            | Wie oben, GMT                                 | updated_at                    | Yes        |
| post_content_filtered | longtext            | Gefilterter Content (nicht verwendet)         | ❌                             | Skip       |
| post_parent           | bigint(20) unsigned | ID des Eltern-Posts                           | parent_id                     | Yes        |
| guid                  | varchar(255)        | ..xmpl.com/?p=123 or ...ntent/uploads/img.jpg | permalink (make host dynamic) | Yes        |
| menu_order            | int(11)             | Reihenfolge (z. B. Pages)                     | order                         | Create     |
| post_type             | varchar(20)         | post, page, attachment, cpt                   | depends on the entity         | Calc       |
| post_mime_type        | varchar(100)        | image/jpeg, application/pdf, video/mp4        | only media                    | Calc       |
| comment_count         | bigint(20)          | Anzahl Kommentare                             | comments counted              | Create     |
...
// goes to metas edit_lock and edit_last then

// Ab Post
$table->uuid('origin_id')->nullable(); 			// original WP ID
comment_status
comment_count
password
order
...
// Discuss how to handle in Moox ... just an idea, not yet
_wp_old_slug (multiple) -> JSON: old_slugs
needs also -> JSON: old_permalinks 
```
...
| --------------------------- | ------------------------------------------------------------ | ----------------------- |
| _thumbnail_id               | Stores the attachment ID of the post’s **featured image**    | Page and Post           |
| _wp_page_template           | Stores the selected **custom page template** (for pages)     | Page only               |
| _wp_attachment_metadata     | Stores **image metadata**, like sizes (for attachments)      | Media only              |
| _wp_attached_file           | Stores **file path** for media files (relative to uploads dir) | Media only              |
| _menu_item_menu_item_parent | Used in **menu items** (custom post type nav_menu_item)      | Navigation only         |
| _menu_item_object_id        | Links a menu item to its target object (e.g., a page ID)     | Navigation only         |
| _edit_lock                  | Prevents concurrent editing by multiple users                | Locked                  |
| _edit_last                  | Stores user ID of last editor                                | Locked                  |
| _wp_old_slug                | Keeps a **slug history** for redirection purposes            |                         |
| _wp_trash_meta_status       | Used when moving posts to the trash (for restore info)       | Restore as draft always |
| _wp_trash_meta_time         | Timestamp of when the post was trashed                       | Deleted_at              |
