...
namespace Moox\Passkey\Commands;
...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'mooxpasskey:install';
     ...
     * @var string
     */
    protected $description = 'Installs Moox Passkey, publishes configuration, migrations and registers plugins.';
    ...
    {
        info('Welcome to Moox Passkey Installer');
    }

    public function publishConfiguration(): void
    {
        if (confirm('Do you wish to publish the configuration?', true)) {
            if (! File::exists('config/passkey.php')) {
                info('Publishing Passkey Configuration...');
                $this->callSilent('vendor:publish', ['--tag' => 'passkey-config']);

                return;
            }
            warning('The Passkey config already exist. The config will not be published.');
        }
    }
    ...
    public function publishMigrations(): void
    {
        if (confirm('Do you wish to publish the migrations?', true)) {
            if (Schema::hasTable('passkeys')) {
                warning('The passkeys table already exists. The migrations will not be published.');

                return;
            }
            info('Publishing Passkeys Migrations...');
            $this->callSilent('vendor:publish', ['--tag' => 'passkey-migrations']);
        }
    }
    ...
    public function runMigrations(): void
    {
        if (confirm('Do you wish to run the migrations?', true)) {
            info('Running Passkey Migrations...');
    ...
    public function registerPlugins(string $providerPath): void
    {
        if (File::exists($providerPath)) {
            ...
            $pluginsToAdd = multiselect(
                ...
                options: ['PasskeyPlugin'],
                default: ['PasskeyPlugin'],
            );
    ...
    {
        note('Moox Passkey installed successfully. Enjoy!');
    }
}
