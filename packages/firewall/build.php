...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i '.basename(__FILE__).' | findstr /r /i /M /F:/ "Firewall firewall"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i "Firewall|firewall" --exclude-dir=vendor ./* | grep -v '.basename(__FILE__)));
}
...
if (! isValidPackageName($folderName)) {
    do {
        writeln('Invalid package name: "firewall" is not allowed.');
...
foreach ($files as $file) {
    replace_in_file($file, [
        'firewall.jpg' => 'made-with-moox.jpg',
        'Moox Developer' => $authorName,
        'dev@moox.org' => $authorEmail,
        'Firewall' => $className,
        'firewall' => $packageSlug,
        'Firewall is a Moox Package using Moox Skeleton.' => $description,
    ...
    match (true) {
        str_contains((string) $file, determineSeparator('src/FirewallServiceProvider.php')) => rename($file, determineSeparator('./src/'.$className.'ServiceProvider.php')),
        str_contains((string) $file, 'README.md') => replace_readme_paragraphs($file, $description),
        default => [],
    };
}

rename(determineSeparator('config/firewall.php'), determineSeparator('./config/'.$packageSlugWithoutPrefix.'.php'));
