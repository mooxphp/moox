...
namespace Moox\UserSession\Commands;
...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Installs Moox UserSession, publishes configuration, migrations and registers plugins.';
    ...
    {
        info('Welcome to Moox UserSession Installer');
    }

    public function publishConfiguration(): void
    {
        if (confirm('Do you wish to publish the configuration?', true)) {
            if (! File::exists('config/user-session.php')) {
                info('Publishing UserSession Configuration...');
                $this->callSilent('vendor:publish', ['--tag' => 'user-session-config']);

                return;
            }
            warning('The UserSession config already exist. The config will not be published.');
        }
    }
    ...
    {
        note('Moox UserSession installed successfully. Enjoy!');
    }
}
