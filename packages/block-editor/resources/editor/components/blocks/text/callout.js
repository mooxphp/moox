 ...
 * Redaktionstauglicher Hinweis-/Alert-Block mit Varianten
 */
...
export const CalloutBlock = {
    ...
    renderHTML(block, context = {}) {
        return this.renderCalloutHTML('block', block, context);
    },

    renderChildHTML(child, context = {}) {
        return this.renderCalloutHTML('child', child, context);
    },

    renderCalloutHTML(scope, data, context = {}) {
        ...
        return `
            <div x-show="${scope}.type === 'callout'">
                <div
                    :id="${scope}.htmlId || null"
                    :style="(${scope}.style || '') + '; ${gridStyle}'"
                    :class="['grid gap-4 p-4 border-2 rounded-lg min-h-[140px]', getCalloutVariantClasses(${scope}.calloutVariant), ${scope}.classes || '']"
                >
                        ...
                        <div class="space-y-2">
                            <template x-for="(child, childIndex) in (column.children || [])" :key="child.id">
                                <div 
                                    ...
                                    class="block-item group relative p-2 rounded-lg hover:bg-white/60 transition-all bg-white/60"
    ...
    initialize(block, blockIdCounter) {
        if (!block.calloutVariant || !CALLOUT_VARIANTS[block.calloutVariant]) {
    ...
    cleanup(block) {
        if (block.calloutVariant !== undefined) {
            delete block.calloutVariant;
        }
        if (block.children) {
    ...
    getSettingsHTML(block, context = {}) {
        ...
        return `
            <div class="pt-4 border-t border-gray-200 space-y-3">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Variante</label>
                    <select
                        x-model="block.calloutVariant"
                        @change="setCalloutVariant(block.id, block.calloutVariant)"
