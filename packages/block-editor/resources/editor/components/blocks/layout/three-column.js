...
export const ThreeColumnBlock = {
    ...
    renderColumnHTML(scope, data, context = {}) {
        const columnCount = Array.isArray(data.children) && data.children.length > 0
        ...
        return `
            <div x-show="${scope}.type === 'threeColumn'" 
                 :id="${scope}.htmlId || null"
                 :style="(${scope}.style || '') + '; ${gridStyle}'"
                 :class="['grid gap-4 p-4 border-2 border-dashed border-gray-300 rounded-lg min-h-[200px]', ${scope}.classes || '']">
                                ...
                                <div :class="selectedBlockId === child.id ? 'opacity-100' : ''" class="absolute -left-16 top-2 flex items-center gap-2 opacity-0 transition-opacity z-10">
                                    <div class="relative" x-data="{ showColumnChildMenu: false }">
                                        ...
                                        <div x-show="showColumnChildMenu" @click.outside="showColumnChildMenu = false" x-transition
                                            ...
                                            <button
                                                type="button" @click.stop="openSidebar(child.id); showColumnChildMenu = false"
                                            ...
                                            <button
                                                type="button" @click.stop="deleteBlock(child.id); showColumnChildMenu = false"
    ...
    initialize(block, blockIdCounter) {
        if (!block.children || block.children.length < 3) {
            block.children = [
                { id: `col-${block.id}-0`, type: 'column', children: [] },
                { id: `col-${block.id}-1`, type: 'column', children: [] },
                { id: `col-${block.id}-2`, type: 'column', children: [] }
            ];
        }
        return block;
    },
    ...
    // Fokus-Verhalten: ThreeColumn ist nicht automatisch fokussierbar
    focusable: false
};
