...
namespace Moox\Frontend\Commands;
...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Installs Moox Frontend, publishes configuration, migrations and registers plugins.';
    ...
    {
        info('Welcome to Moox Frontend Installer');
    }

    public function publishConfiguration(): void
    {
        if (confirm('Do you wish to publish the configuration?', true)) {
            if (! File::exists('config/frontend.php')) {
                info('Publishing Frontend Configuration...');
                $this->callSilent('vendor:publish', ['--tag' => 'frontend-config']);

                return;
            }

            warning('The Frontend config already exist. The config will not be published.');
        }
    }
    ...
    public function publishMigrations(): void
    {
        if (confirm('Do you wish to publish the migrations?', true)) {
            if (Schema::hasTable('frontend')) {
                warning('The frontend table already exists. The migrations will not be published.');

                return;
            }

            info('Publishing Frontend Migrations...');
            $this->callSilent('vendor:publish', ['--tag' => 'frontend-migrations']);
        }
    }
    ...
    public function runMigrations(): void
    {
        if (confirm('Do you wish to run the migrations?', true)) {
            info('Running Frontend Migrations...');
    ...
    {
        note('Moox Frontend installed successfully. Enjoy!');
    }
}
