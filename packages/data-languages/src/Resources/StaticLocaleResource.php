...
class StaticLocaleResource extends Resource
{
    ...
    protected static ?string $model = \Moox\DataLanguages\Models\StaticLocale::class;

    protected static ?string $navigationIcon = 'gmdi-fmd-good-s';

    public static function getModelLabel(): string
    {
        return __('data-languages::static-locale.static_locale');
    }

    public static function getPluralModelLabel(): string
    {
        return __('data-languages::static-locale.static_locales');
    }
                            ...
                            Section::make()
                                ->schema([
                                    Select::make('language_id')
                                        ->label(__('data-languages::static-locale.language'))
                                        ->relationship('language', 'common_name')
                                        ->searchable()
                                        ->preload()->required(),
                                    Select::make('country_id')
                                        ->label(__('data-languages::static-locale.country'))
                                        ->relationship('country', 'common_name')
                                        ->searchable()
                                        ->preload()->required(),
                                    Toggle::make('is_official_language')
                                        ->label(__('data-languages::static-locale.is_official_language'))
                                        ->default(false),
                                    TextInput::make('locale')
                                        ->label(__('data-languages::static-locale.locale'))
                                        ->maxLength(255)->required(),
                                    TextInput::make('name')
                                        ->label(__('data-languages::static-locale.name'))
                                        ->maxLength(255)->required(),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                TextColumn::make('locale')->label(__('data-languages::static-locale.locale')),
                TextColumn::make('name')->label(__('data-languages::static-locale.name'))->sortable()->searchable()->toggleable(),
                IconColumn::make('is_official_language')
                    ->label(__('data-languages::static-locale.is_official_language'))
                    ->boolean(),
                TextColumn::make('language.common_name')
                    ->label(__('data-languages::static-locale.common_language_name'))
                    ->sortable(),
                TextColumn::make('country.common_name')
                    ->label(__('data-languages::static-locale.common_country_name'))
                    ->sortable(),
            ])
            ->defaultSort('id', 'desc')
            ...
            ->filters([
                Filter::make('id')
                    ->form([
                        TextInput::make('id')
                            ->label(__('data-languages::static-locale.id'))
                ...
                Filter::make('language_id')
                    ->form([
                        TextInput::make('language_id')
                            ->label(__('data-languages::static-locale.language_id'))
                            ->placeholder(__('core::core.search')),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                ...
                Filter::make('country_id')
                    ->form([
                        TextInput::make('country_id')
                            ->label(__('data-languages::static-locale.country_id'))
                            ->placeholder(__('core::core.search')),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
