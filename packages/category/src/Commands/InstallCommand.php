...
namespace Moox\Category\Commands;
...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'mooxcategory:install';
     ...
     * @var string
     */
    protected $description = 'Installs Moox Category, publishes configuration, migrations and registers plugins.';
    ...
    {
        info('Welcome to Moox Category Installer');
    }

    public function publishConfiguration(): void
    {
        if (confirm('Do you wish to publish the configuration?', true)) {
            if (! File::exists('config/category.php')) {
                info('Publishing Category Configuration...');
                $this->callSilent('vendor:publish', ['--tag' => 'category-config']);

                return;
            }

            warning('The Category config already exist. The config will not be published.');
        }
    }
    ...
    public function publishMigrations(): void
    {
        if (confirm('Do you wish to publish the migrations?', true)) {
            if (Schema::hasTable('categories')) {
                warning('The categories table already exists. The migrations will not be published.');

                return;
            }

            info('Publishing Categorys Migrations...');
            $this->callSilent('vendor:publish', ['--tag' => 'category-migrations']);
        }
    }
    ...
    public function runMigrations(): void
    {
        if (confirm('Do you wish to run the migrations?', true)) {
            info('Running Category Migrations...');
    ...
    public function registerPlugins(string $providerPath): void
    {
        if (File::exists($providerPath)) {
            ...
            $pluginsToAdd = multiselect(
                ...
                options: ['CategoryPlugin'],
                default: ['CategoryPlugin'],
            );
    ...
    {
        note('Moox Category installed successfully. Enjoy!');
    }
}
