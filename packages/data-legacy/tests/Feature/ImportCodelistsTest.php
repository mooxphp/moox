...
use Moox\DataLegacy\Tests\TestCase;

uses(TestCase::class);

test('moox:data-legacy:import-codelists populates all codelist tables from committed JSON', function (): void {
    $this->artisan('moox:data-legacy:import-codelists')
        ->assertSuccessful();
...
test('vatex import maps description and vat_category_code and vatCategory resolves by code', function (): void {
    $this->artisan('moox:data-legacy:import-codelists', ['scheme' => 'untdid5305'])
        ->assertSuccessful();
    $this->artisan('moox:data-legacy:import-codelists', ['scheme' => 'vatex'])
        ->assertSuccessful();
...
test('icd import maps description when present in JSON', function (): void {
    $this->artisan('moox:data-legacy:import-codelists', ['scheme' => 'icd'])
        ->assertSuccessful();
