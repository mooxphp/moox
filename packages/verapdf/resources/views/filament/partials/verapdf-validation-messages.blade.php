@php
    /** @var \Moox\VeraPdf\Models\VeraPdfValidation $record */
    $raw = is_array($record->errors) ? $record->errors : [];
    $messages = [];
    foreach ($raw as $item) {
        if (! is_array($item)) {
            continue;
        ...
        $messages[] = [
            ...
            'rule' => is_string($item['rule'] ?? null) ? $item['rule'] : null,
        ];
    }
    ...
    $severityChipClasses = static fn (string $type): string => match ($type) {
        ...
        default => 'inline-flex items-center rounded-md border border-gray-500/10 bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700 dark:border-gray-500/15 dark:bg-gray-500/10 dark:text-gray-300',
    };
@endphp
@if ($messages === [])
    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('verapdf::fields.no_validation_messages') }}</p>
@else
                            ...
                            <li class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                                ...
                                @if (filled($msg['location']) || filled($msg['rule']))
                                    <p class="mt-1 break-words text-xs text-gray-500 dark:text-gray-400">
                                        @if (filled($msg['rule']))
                                            <span>{{ __('verapdf::fields.rule') }}: {{ $msg['rule'] }}</span>
                                        @endif
                                        @if (filled($msg['location']))
                                            <span class="{{ filled($msg['rule']) ? 'ml-2' : '' }}">{{ __('verapdf::fields.location') }}: {{ $msg['location'] }}</span>
                                        @endif
                                    </p>
                                @endif
                            </li>
