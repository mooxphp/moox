...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'item:install';
     ...
     * @var string
     */
    protected $description = 'Installs Moox Item, publishes configuration, migrations and registers plugins.';
    ...
    {
        info('Welcome to Moox Item Installer');
    }

    public function publishConfiguration(): void
    {
        if (confirm('Do you wish to publish the configuration?', true)) {
            if (! File::exists('config/item.php')) {
                info('Publishing Item Configuration...');
                $this->callSilent('vendor:publish', ['--tag' => 'item-config']);

                return;
            }

            warning('The Item config already exist. The config will not be published.');
        }
    }
    ...
    public function publishMigrations(): void
    {
        if (confirm('Do you wish to publish the migrations?', true)) {
            if (Schema::hasTable('item')) {
                warning('The item table already exists. The migrations will not be published.');

                return;
            }

            info('Publishing Item Migrations...');
            $this->callSilent('vendor:publish', ['--tag' => 'item-migrations']);
        }
    }
    ...
    {
        note('Moox Item installed successfully. Enjoy!');
    }
}
