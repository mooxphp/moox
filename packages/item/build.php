...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i '.basename(__FILE__).' | findstr /r /i /M /F:/ "Item item"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i "Item|item" --exclude-dir=vendor ./* | grep -v '.basename(__FILE__)));
}
...
if (! isValidPackageName($folderName)) {
    do {
        writeln('Invalid package name: "item" is not allowed.');
...
foreach ($files as $file) {
    ...
    match (true) {
        str_contains((string) $file, determineSeparator('src/ItemServiceProvider.php')) => rename($file, determineSeparator('./src/'.$className.'ServiceProvider.php')),
        str_contains((string) $file, 'README.md') => replace_readme_paragraphs($file, $description),
        default => [],
    };
}

rename(determineSeparator('config/item.php'), determineSeparator('./config/'.$packageSlugWithoutPrefix.'.php'));
