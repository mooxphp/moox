...
Parent and children tabs are generated by `ConfigRelationManager` from `config('company.relations')` (`kind: belongs_to` / `has_many`). Create on the children tab opens the create page with `parent_id` pre-filled via `create_prefill`:
...
| `relations` | Config-driven tabs (parent, children, contacts, addressables, …) |
