...
There is a new service `ConfigTranslator` that should handle this, but it does only work for packages and must do app translations as well.

We use the `TranslatableConfig` trait in the `PreviewSimItem`, and call the config keys there.
We have a config in `config/previews/preview-sim-item.php` that is used by the `PreviewSimItem` to generate the config for the preview.
We have a translation in `lang/previews/preview-sim-item.php` that is used by the `PreviewSimItem` to generate the translations for the preview. As that is not working yet, we also did a second translation file `lang/preview-sim-item.php`, to simply have a second debug option.

We need translations in config files. Don't suggest the direct translation approach, as WE JUST NEED!
We need to stick with the default Laravel way of doing translations. Don't suggest a custom translation implementation.
We need to the it working in App context as well. Packages work fine, so the approach could not be that wrong ... I hope.

We discussed and tried a lot of things, including using closures instead, but none of them worked.

Current Status: for all app translations we just get the untranslated string back, packages work.

Packages config and translations are for example in `builder.php` config file and the translation in this package. There are some test resources also in builder and maybe 20 more packages that are all working, just like builder does.

The `PreviewSimItem` is generated by Builder btw.

We have not yet checked Laravel's translation debug log, how to...?
