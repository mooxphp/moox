@php
    $body = filled($content ?? null) ? $content : __('login-link::translations.mail_intro');
@endphp
                    ...
                    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px;">
                        <tr>
                            <td align="center" style="padding:0 0 20px;">
                                @if(filled($logoUrl ?? null))
                                    <img src="{{ $logoUrl }}" alt="{{ $brandName ?? config('app.name') }}" style="height:44px; width:auto; display:block;">
                                @else
                        ...
                        <tr>
                            <td style="background:#ffffff; border-radius:12px; padding:24px; box-shadow: 0 1px 2px rgba(16,24,40,.06);">
                                ...
                                <div style="margin:0 0 16px; font-size:14px; line-height:22px; color:#374151; white-space:pre-wrap;">{{ $body }}</div>
