...
- **Format binding.** Customer preference lives on the `customers` row (`preferred_ebilling_format`, written by the portal settings UI). Each document **freezes** the format actually produced in a real `ebilling_documents.format` column. Resolution: `document.format` ← `customers.preferred_ebilling_format` ← global default (**ZUGFeRD / EN16931**). Preference changes affect **future** documents only.
