...
- **Company name matching.** ✅ One `CompanyNameMatcher` serves field validation and format resolution. Both match on the persisted invoice buyer name (whitespace-collapsed, case-insensitive exact equality, unique hit only); `bill_data['customer_name']` is not used.
