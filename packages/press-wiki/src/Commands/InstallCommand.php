...
namespace Moox\PressWiki\Commands;
...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'mooxpress-wiki:install';
     ...
     * @var string
     */
    protected $description = 'Installs Moox PressWiki, publishes configuration, migrations and registers plugins.';
    ...
    {
        info('Welcome to Moox PressWiki Installer');
    }
    ...
    public function publishConfiguration(): void
    {
        if (confirm('Do you wish to publish the configuration?', true)) {
            if (! File::exists('config/moox-press-wiki.php')) {
                info('Publishing PressWiki Configuration...');
                $this->call('vendor:publish', ['--tag' => 'press-wiki-config']);

                return;
            }
            warning('The PressWiki config already exist. The config will not be published.');
        }
    }
    ...
    {
        note('Moox PressWiki installed successfully. Enjoy!');
    }
}
