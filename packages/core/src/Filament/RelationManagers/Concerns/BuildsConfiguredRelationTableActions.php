...
trait BuildsConfiguredRelationTableActions
{
    ...
    protected function buildConfiguredAction(ResolvedRelation $resolved, RelationTabAction $action, bool $record = false): mixed
    {
        return match ($action) {
            ...
            RelationTabAction::Dissociate => $record ? $this->buildDissociateAction($resolved) : null,
            RelationTabAction::Detach => $record ? DetachAction::make() : null,
    ...
    protected function buildDissociateAction(ResolvedRelation $resolved): DissociateAction|Action
    {
    ...
    protected function searchRelatedRecords(ResolvedRelation $resolved, string $search, array $options = []): array
    {
        ...
        if (
            ...
            $invalidIds = $owner->{$resolved->inverseRelationship}()->pluck($owner->getKeyName());
