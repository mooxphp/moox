...
class WP_Theme_JSON
{
    ...
    public function get_stylesheet($types = ['variables', 'styles', 'presets'], $origins = null, $options = [])
    {
        ...
        if (in_array('styles', $types, true)) {
            if ($root_style_key !== false && empty($options['skip_root_layout_styles'])) {
            ...
            foreach ($layout_definitions as $layout_definition) {
                ...
                if (
                    ...
                    foreach ($base_style_rules as $base_style_rule) {
                        ...
                        if (in_array('base-layout-styles', $types, true) && ($layout_definition['name'] === 'default' || $layout_definition['name'] === 'constrained')) {
                            continue;
                        }
    ...
    protected static function compute_style_properties($styles, $settings = [], $properties = null, $theme_json = null, $selector = null, $use_root_padding = null)
    {
        ...
        foreach ($properties as $css_property => $value_path) {
             ...
             */
            if ($css_property === 'background-image' && ! empty($value)) {
    ...
    public function get_styles_for_block($block_metadata)
    {
        ...
        foreach ($declarations as $index => $declaration) {
            if ($declaration['name'] === 'filter') {
                 ...
                 * than 'unset'.
                 */
                if ($declaration['value'] !== 'unset') {
                    $declarations_duotone[] = $declaration;
                }
                unset($declarations[$index]);
            }

            if ($is_root_selector && ($declaration['name'] === 'background-image' || $declaration['name'] === 'background')) {
        ...
        foreach ($nodes as $node) {
            ...
            foreach (static::PRESETS_METADATA as $preset_metadata) {
                ...
                foreach (static::VALID_ORIGINS as $origin) {
                    ...
                    if ($origin === 'theme' && $preset_metadata['use_default_names']) {
                        ...
                        foreach ($valid_element_names as $element_name) {
                            ...
                            if ($element_input) {
                                ...
                                if (! empty($element_output)) {
                                    _wp_array_set($variation_output, ['elements', $element_name], $element_output);
                                }
                            }
                        }
                    }
