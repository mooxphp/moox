...
function update_ignored_hooked_blocks_postmeta($post)
{
     ...
     */
    if (! isset($post->post_type) || $post->post_type !== 'wp_navigation') {
        return $post;
    }
...
function resolve_pattern_blocks($blocks)
{
    ...
    while ($i < count($blocks)) {
        if ($blocks[$i]['blockName'] === 'core/pattern') {
...
function filter_block_kses_value($value, $allowed_html, $allowed_protocols = [], $block_context = null)
{
    if (is_array($value)) {
        foreach ($value as $key => $inner_value) {
            ...
            if (isset($block_context['blockName']) && $block_context['blockName'] === 'core/template-part') {
...
{
    if (empty($attribute_value) || $attribute_name !== 'tagName') {
