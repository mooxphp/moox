...
final class WP_Interactivity_API
{
     ...
     */
    public function state(string $store_namespace, array $state = []): array
    {
    ...
    public function add_hooks()
    {
        ...
        add_action('wp_footer', [$this, 'print_client_interactivity_data']);
    }
    ...
    private function process_directives_args(string $html, array &$context_stack, array &$namespace_stack)
    {
        ...
        while ($p->next_tag(['tag_closers' => 'visit'])) {
            ...
            if ($p->is_tag_closer()) {
                ...
                if (count($tag_stack) === 0 || $opening_tag_name !== $tag_name) {
                ...
                if (count($p->get_attribute_names_with_prefix('data-wp-each-child')) !== 0) {
                    /*
             ...
             * processing.
             */
            if (count($directives_prefixes) === 0) {
                continue;
            }
            ...
            foreach ($modes as $mode => $should_run) {
                ...
                $existing_directives_prefixes = array_intersect(
                    $mode === 'enter' ? $directive_processor_prefixes : $directive_processor_prefixes_reversed,
                    $directives_prefixes
                );
                foreach ($existing_directives_prefixes as $directive_prefix) {
                    ...
                    call_user_func_array(
                        $func,
                        [$p, $mode, &$context_stack, &$namespace_stack, &$tag_stack]
                    );
                }
            }
        }
         ...
         */
        return $unbalanced || count($tag_stack) > 0 ? null : $p->get_updated_html();
    }
    ...
    private function extract_directive_value($directive_value, $default_namespace = null): array
    {
        ...
        if (preg_match('/^([\w\-_\/]+)::./', $directive_value) === 1) {
        ...
        if ($decoded_json !== null || $directive_value === 'null') {
    ...
    private function data_wp_interactive_processor(WP_Interactivity_API_Directives_Processor $p, string $mode, array &$context_stack, array &$namespace_stack)
    {
        ...
        $namespace_stack[] = ($new_namespace && preg_match('/^([\w\-_\/]+)/', $new_namespace) === 1)
            ? $new_namespace
            : end($namespace_stack);
    }
    ...
    private function data_wp_context_processor(WP_Interactivity_API_Directives_Processor $p, string $mode, array &$context_stack, array &$namespace_stack)
    {
        ...
        if (is_string($namespace_value)) {
            $context_stack[] = array_replace_recursive(
                end($context_stack) !== false ? end($context_stack) : [],
...
HTML;
    ...
    {
        if ($mode === 'enter' && ! $this->has_processed_router_region) {
            ...
            add_action('wp_footer', [$this, 'print_router_loading_and_screen_reader_markup']);
        }
    }
    ...
    {
        if ($mode === 'enter' && $p->get_tag() === 'TEMPLATE') {
