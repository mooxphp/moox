...
class WP_Script_Modules
{
        ...
        if (! isset($this->registered[$id])) {
            ...
            foreach ($deps as $dependency) {
                if (is_array($dependency)) {
                    ...
                    $dependencies[] = [
                        'id' => $dependency['id'],
                        'import' => isset($dependency['import']) && $dependency['import'] === 'dynamic' ? 'dynamic' : 'static',
                    ];
    ...
    private function get_src(string $id): string
    {
        ...
        if ($script_module['version'] === false) {
            $src = add_query_arg('ver', get_bloginfo('version'), $src);
        } elseif ($script_module['version'] !== null) {
