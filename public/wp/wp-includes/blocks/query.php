...
function block_core_query_disable_enhanced_pagination($parsed_block)
{
    ...
    $has_enhanced_pagination = isset($parsed_block['attrs']['enhancedPagination']) && $parsed_block['attrs']['enhancedPagination'] === true && isset($parsed_block['attrs']['queryId']);
    /*
     * Client side navigation can be true in two states:
     *  - supports.interactivity = true;
     *  - supports.interactivity.clientNavigation = true;
     */
    $supports_client_navigation = (isset($block_type->supports['interactivity']['clientNavigation']) && $block_type->supports['interactivity']['clientNavigation'] === true)
        || (isset($block_type->supports['interactivity']) && $block_type->supports['interactivity'] === true);

    if ($block_name === 'core/query' && $has_enhanced_pagination) {
        ...
        if (! isset($render_query_callback)) {
            ...
            $render_query_callback = static function ($content, $block) use (&$enhanced_query_stack, &$dirty_enhanced_queries, &$render_query_callback) {
                $has_enhanced_pagination = isset($block['attrs']['enhancedPagination']) && $block['attrs']['enhancedPagination'] === true && isset($block['attrs']['queryId']);
